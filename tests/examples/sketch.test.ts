...
const SAVE_FOLDER = './tests/outputs/sketch/';
...
describe('Sketch', () =>
{
    it('should create basic curves on XY', () =>
    {
        ...
        expect(pline.curves().length).toBe(1);
        ...
        expect(pline.first().close().length()).toBeCloseTo(40);
    });
    ...
    it('make lines with polar coordinates', async () =>
    {
        ...
        await save(`${SAVE_FOLDER}test.sketch.polar.gltf`,
            await new ShapeCollection<Curve>(plortho, pl, pl2, pl3).toGLTF());
    });
});
