...
impl<S: Clone + Debug + Send + Sync> Sketch<S> {
    ...
    fn extrude_geometry(
        ...
        // When the extrusion opposes the sketch-plane normal (+Z),
        // the solid is inverted and face orientations must flip.
        ...
        match geom {
            geo::Geometry::Polygon(poly) => {
                ...
                // bottom (un-translated sketch face)
                for tri in &tris {
                    let (a, b, c) = if flip {
                        (tri[0], tri[1], tri[2])
                    } else {
