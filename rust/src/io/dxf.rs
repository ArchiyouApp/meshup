...
impl<S: Clone + Debug + Send + Sync> Sketch<S> {
    /// Import **2-D** geometry from DXF as a Sketch of curves.
    ///
    /// LINE / ARC / open (LW)POLYLINE become open `LineString`s; CIRCLE and
    /// closed (LW)POLYLINE become closed `Polygon`s. 3-D `SOLID`s are ignored —
    /// use [`Mesh::from_dxf`] for those.
    pub fn from_dxf(dxf_data: &[u8], metadata: Option<S>) -> Result<Sketch<S>, Box<dyn Error>> {
        use geo::{Geometry, GeometryCollection, LineString};

        let drawing = Drawing::load(&mut Cursor::new(dxf_data))?;
        let mut geoms: Vec<Geometry<Real>> = Vec::new();
        let seg = 48usize;

        let push_ring = |geoms: &mut Vec<Geometry<Real>>, pts: Vec<(Real, Real)>, closed: bool| {
            if pts.len() < 2 {
                return;
            }
            let ls = LineString::from(pts);
            if closed {
                geoms.push(Geometry::Polygon(GeoPolygon::new(ls, Vec::new())));
            } else {
            ...
            match &entity.specific {
                EntityType::Line(line) => {
                    push_ring(
                        &mut geoms,
                        vec![
                            (line.p1.x as Real, line.p1.y as Real),
                            (line.p2.x as Real, line.p2.y as Real),
                        ],
                        false,
                    );
                }
                EntityType::LwPolyline(lwp) => {
                    let pts: Vec<(Real, Real)> =
                        lwp.vertices.iter().map(|v| (v.x as Real, v.y as Real)).collect();
                    push_ring(&mut geoms, pts, lwp.is_closed());
                }
                EntityType::Polyline(poly) => {
                    let pts: Vec<(Real, Real)> = poly
                        .vertices()
                        .map(|v| (v.location.x as Real, v.location.y as Real))
                        .collect();
                    push_ring(&mut geoms, pts, poly.is_closed());
                }
                EntityType::Circle(circle) => {
                    let (cx, cy, r) =
                        (circle.center.x as Real, circle.center.y as Real, circle.radius as Real);
                    let pts: Vec<(Real, Real)> = (0..=seg)
                        .map(|i| {
                            let t = 2.0 * crate::float_types::PI * (i as Real) / (seg as Real);
                            (cx + r * t.cos(), cy + r * t.sin())
                        })
                        .collect();
                    push_ring(&mut geoms, pts, true);
                }
                EntityType::Arc(arc) => {
                    let (cx, cy, r) =
                        (arc.center.x as Real, arc.center.y as Real, arc.radius as Real);
                    let a0 = arc.start_angle as Real * crate::float_types::PI / 180.0;
                    let mut a1 = arc.end_angle as Real * crate::float_types::PI / 180.0;
                    if a1 <= a0 {
                        a1 += 2.0 * crate::float_types::PI;
                    }
                    let pts: Vec<(Real, Real)> = (0..=seg)
                        .map(|i| {
                            let t = a0 + (a1 - a0) * (i as Real) / (seg as Real);
                            (cx + r * t.cos(), cy + r * t.sin())
                        })
                        .collect();
                    push_ring(&mut geoms, pts, false);
                }
                _ => {}
            }
        }

        Ok(Sketch::from_geo(GeometryCollection(geoms), metadata))
    }
}
